...
fn main() {
    ...
    //step 1 - get grid into vector or char vector
    ...
    for (index, y_line) in grid.iter().enumerate() {
        ...
        if row_is_empty == 1 {grid_with_expanded_rows.push(y_line.to_vec())} 
    }
    ...
    //expand rows again
    let mut grid_with_expanded_columns: Vec<Vec<char>> = Vec::new();
    ...
    for (index, y_line) in grid_with_expanded_rows__pivot.iter().enumerate() {
        ...
        if row_is_empty == 1 {grid_with_expanded_columns.push(y_line.to_vec())} 
    }
    ...
    for (index, y_line) in expended_grid.iter().enumerate() {
        ...
        for (index, point) in y_line.iter().enumerate() {
            ...
            if *point != '.' {
                ...
                let mut this_galaxy: Vec<i32> = Vec::new();
                let y_axis_int:i32 = y_axis.try_into().unwrap();
                let x_axis_int:i32 = x_axis.try_into().unwrap();

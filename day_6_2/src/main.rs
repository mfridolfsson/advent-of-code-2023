...
fn main() {
    ...
    let times_just_data = re.replace_all(times_just_data ,"");

    let re = Regex::new(r"[^0-9.]").unwrap();
    let distance_just_data = re.replace_all(distance_just_data ,"");
    ...
    for n in 0..=loop_count {
        let this_time:i64 = times_clean_vec[n].parse::<i64>().unwrap();
        let this_distance:i64 = distances_clean_vec[n].parse::<i64>().unwrap();
        let mut win_count = 0;
